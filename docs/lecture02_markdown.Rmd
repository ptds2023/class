...
1. Change the output format to `github_document`.
1. Create a new header of type 2.
1. Make a linear regression with "Sepal Length" as a response and 
"Sepal Width" as an explanatory variable from the `iris` dataset and save the result. Highlight the code with `monochrome` style.
...
1. Remove the `.` from the lables (click [here](https://bookdown.org/yihui/rmarkdown-cookbook/kable.html#change-column-names))
